...
from coalescent_utils import compute_coalescent_scores
...
def run_for_window(returns, window_days):
    ...
    try:
        raw_scores = compute_coalescent_scores(ret_window, metric=config.DISTANCE_METRIC, linkage_method=config.LINKAGE_METHOD)
    except Exception as e:
        print(f"    Error: {e}")
        return None
    norm_scores = normalize_scores(raw_scores)
    sorted_norm = sorted(norm_scores.items(), key=lambda x: x[1], reverse=True)
    top_etfs = [{"ticker": t, "coalescent_score_norm": s, "raw_score": raw_scores[t]} for t, s in sorted_norm[:config.TOP_N]]
    return {
        "window": window_days,
        "top_etfs": top_etfs,
